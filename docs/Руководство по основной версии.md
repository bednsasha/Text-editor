...
Наш **текстовый редактор будет состоять из 3 частей**, *меню* в верхней части окна, *текстового поля* и *полосы прокрутки - скроллбара*. В свою часть в меню включим 2 выпадающих списка, для работы с документом и для настройки вида документа, а также текстовое поле для отображения текущего имени документа.
...
Установление параметра **yscrollcommand=scroll.set** позволяет связать скроллер (полосу прокрутки) с текстовым виджетом.
...
Изначально, до того, как пользователь откроет, либо создаст документ в поле задан текст (статус): "Новый документ". Текст должен изменяться на имя файла при открытии, либо создании. 
...
+ "Новый документ" и пустое текстовое поле или отсутвие изменений в текущем документе -  закрытие программы
+ "Новый документ" и наличие текста в текстовом поле или наличие изменений в текущем документе - предупреждение о несохраненных изменениях
...
С помощью add_cascade определяем, что в main_menu будут содержаться 2 выпадающий списка "Файл" и "Вид", на основе file_menu и view_menu соотвественно.
